    ...
    <div className="min-h-screen bg-cream flex" dir="rtl">
      ...
      <aside className="w-60 flex-shrink-0 bg-white border-l border-beige flex flex-col">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          <NavItem href="/admin" icon="ti-dashboard" label="لوحة التحكم" />
          <NavItem href="/admin/pipeline" icon="ti-robot" label="خط الإنتاج" />
          <NavItem href="/admin/costs" icon="ti-chart-bar" label="التكاليف" />
...
function NavItem({
  ...
  // Simple active check based on pathname — using window.location for client component
  return (
    <a
      href={href}
      target={external ? '_blank' : undefined}
      rel={external ? 'noopener noreferrer' : undefined}
      className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-stone hover:bg-sage/5 hover:text-charcoal transition-colors group"
    >
